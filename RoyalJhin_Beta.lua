...
local RTarget = TargetSelector(3000,TARGET_LOw_HP,DAMAGE_PHYSICAL,true,false)
local target1 = TargetSelector(2500,TARGET_PRIORITY,DAMAGE_PHYSICAL,true,false)
...
OnTick(function(myHero)	
	...
	if RCasting and ValidTarget(ULTTarget, 3000) then
      local R1Pred = GetPredictionForPlayer(GetOrigin(myHero),ULTTarget,GetMoveSpeed(ULTTarget),1400,250,3000,220,false,true)
				if R1Pred.HitChance == 1 and GetDistance(ULTTarget) >= 3000 then
					CastSkillShot(_R, R1Pred.PredPos)
					end
      elseif RCast == 1 then
      local R2Pred = GetPredictionForPlayer(GetOrigin(myHero),ULTTarget,GetMoveSpeed(ULTTarget),1400,250,3000,220,false,true)
				if R2Pred.HitChance == 1 then
					CastSkillShot(_R, R2Pred.PredPos)
					end
      elseif RCast == 2 then
      local R3Pred = GetPredictionForPlayer(GetOrigin(myHero),ULTTarget,GetMoveSpeed(ULTTarget),1400,250,3000,220,false,true)
				if R3Pred.HitChance == 1 then
					CastSkillShot(_R, R3Pred.PredPos)
					end
	  elseif RCast == 3 then
      local R4Pred = GetPredictionForPlayer(GetOrigin(myHero),ULTTarget,GetMoveSpeed(ULTTarget),1400,250,3000,220,false,true)
	...
	if Yomuu >= 1 and ValidTarget(target,550) and JhinMenu.Misc.UseYoumuu:Value() then
        ...
        elseif IsReady(_W) and JhinMenu.Combo.WSettings.W:Value() and (GetPercentMP(myHero) >= JhinMenu.Combo.WSettings.WMana:Value()) then
			if IsMarked == true and IsReady(_W) and ValidTarget(target, 2500) then
				local WPred = GetPredictionForPlayer(GetOrigin(myHero),target,GetMoveSpeed(target),1400,250,2500,220,false,true)
...
OnProcessSpell(function(unit,spell)
  if unit == myHero then
    ...
    if spell.name:lower():find("jhinr") then
    IOW.movementEnabled = false
    IOW.attacksEnabled = false
    IsChanneled = true
    RCasting = true
	end
	if spell.name:lower():find("jhinrshotmis4") then
    IOW.movementEnabled = true
    IOW.attacksEnabled = true
    IsChanneled = false
    RCasting = false
	end
	if spell.name:lower():find("jhinrshotmis") then
	RCast = RCast + 1
	end
  end
end)
